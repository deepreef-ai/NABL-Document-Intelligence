...
def _legacy_path(monkeypatch):
    """These tests describe the LEGACY extraction path, so pin it on.

    process_document now routes to the LangGraph workflow (app/graph/) by
    default, which mocks none of the stubs below and reads a real file — so
    every assertion here started measuring a code path it was never written
    for, and the whole module failed for a reason that had nothing to do with
    what it tests.

    graph_pipeline_enabled is a documented rollback lever, so the legacy path
    is still reachable and still worth having covered; this fixture is what
    keeps that cover honest rather than accidental.
    """
    from app.graph import config as graph_config

    settings = graph_config.get_graph_settings().model_copy(update={"graph_pipeline_enabled": False})
    monkeypatch.setattr(graph_config, "get_graph_settings", lambda: settings)
    monkeypatch.setattr(pipeline, "get_graph_settings", lambda: settings, raising=False)
...
def test_english_image_prefers_local_rapidocr_over_vision(monkeypatch):
    ...
    assert calls["vision"] == 0
    # By name, not by index: the result also carries empty placeholders for the
    # schema slots the document did not fill, and which of those sorts first is
    # not what this test is about. The point is that the value RapidOCR read
    # kept the rectangle RapidOCR found it in, so the reviewer can see it
    # highlighted on the page.
    gst = next(f for f in result.fields if f.field == "organisation.gst_number")
    assert gst.source_bbox == Rect(x=0, y=0, w=10, h=5)
