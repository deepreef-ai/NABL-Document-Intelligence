...
def _normalize_value(value: Any) -> str | None:
    """Fold the ways two transcriptions of the SAME cell can differ.

    Every rule here was measured on this corpus, and each earns its place by
    folding a difference that is about typography rather than about reading:

    - NFKC, because a PDF text layer emits full-width characters and
      non-breaking spaces where the page shows ordinary ones.
    - Dates first, while their separators are still intact: "02/11/2020" and
      "02-11-2020" are one date, and the later rules would destroy the
      structure `_canonical_date` needs to see it.
    - Unicode dashes to a plain hyphen — en dash, em dash and minus are used
      interchangeably by OCR.
    - Separator punctuation (, | / ;) DROPPED. Both sides invent separators:
      the page puts address parts on separate lines, the labeler wrote commas,
      the model wrote slashes. A comma present on one side and absent on the
      other was the single most common punctuation-only mismatch here
      (lab_address, lab_website, client_address). "." and "-" are deliberately
      NOT dropped — they carry numeric meaning, and removing them would
      collapse "6.5" to "65" and "40-129" to "40129".
    - ALL internal whitespace, not merely runs of it: "40 - 129" and "40-129",
      "02 Nov 2020" and "02Nov2020" are the same value, and OCR is inconsistent
      about where it inserts a space around a dash or drops one between words.
    - Trailing sentence punctuation: a labeler transcribing "EDTA Blood." and
      a model returning "EDTA Blood" read the same cell. Safe because it can
      only strip terminal punctuation — unlike a containment or prefix rule,
      which was measured and rejected for wrongly crediting a dropped date
    ...
    canonical = _canonical_date(text)
    if canonical:
        return canonical
